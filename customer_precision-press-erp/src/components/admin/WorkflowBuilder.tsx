...
import { StaffRole, ROLE_META, ALL_STAFF_ROLES } from '@/types/roles';
...
export function WorkflowBuilder({ steps = [], onChange }: WorkflowBuilderProps) {
  ...
  const addStep = (role: StaffRole) => {
    ...
    if (!meta) return; // Skip if role metadata doesn't exist
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {steps.length === 0 ? (
          ...
          steps
            .filter(step => ROLE_META[step.role]) // Only show steps with valid metadata
            .map((step, index) => {
            const meta = ROLE_META[step.role];
            if (!meta) return null; // Skip rendering if role metadata doesn't exist
            return (
              <div 
                key={step.id}
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {ALL_STAFF_ROLES
            .filter(role => !['SUPER_ADMIN', 'ADMIN', 'SUPPORT'].includes(role) && ROLE_META[role])
            .map(role => {
              const meta = ROLE_META[role];
              if (!meta) return null;
              return (
