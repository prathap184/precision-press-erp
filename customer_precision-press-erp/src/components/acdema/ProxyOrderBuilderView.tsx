            ...
            <div className="grid gap-6 lg:grid-cols-12">
              <div className="lg:col-span-5 lg:col-start-8">
                ...
                <div className="rounded-[2rem] bg-white/50 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-2xl border border-white/60">
                  ...
                  {paymentMethodTab === 'CASH_UPI' && paymentMode === 'UPI' && (
                    <div className="mb-4">
                      <label className="flex h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 text-xs font-bold text-slate-500 hover:bg-slate-100 transition-colors">
                        <Upload size={16} />
                        {upiUploading ? 'Uploading...' : 'Upload UPI Proof'}
                        <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) handleUpload(file);
                        }} />
                      </label>
                      {upiPreview && <img src={upiPreview} alt="Proof" className="mt-3 h-20 rounded-lg object-cover" />}
                    </div>
                  )}
