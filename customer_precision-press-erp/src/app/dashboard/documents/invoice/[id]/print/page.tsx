...
import { supabase } from '@/lib/supabase'; // Used only for fallback profile fetching if needed, though mostly handled by snap
...
export default function CustomerInvoicePrintPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/invoices?idOrNumber=${id}&single=true`)
      .then(res => res.json())
      .then((data) => {
        if (data && !data.error) {
          setInvoice(data);
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen bg-slate-100"><Loader2 className="animate-spin text-indigo-500 w-8 h-8" /></div>;
  ...
  const c_snap = parseJson(invoice.customer_snapshot);
  const t_snap = parseJson(invoice.company_snapshot);
  const d_snap = parseJson(invoice.dispatch_details_snapshot);
  ...
  const dbItems = parseJson(invoice.items) || [];
  const mappedItems: InvoiceItem[] = Array.isArray(dbItems) ? dbItems.map((it: any) => ({
    sr: it.sr,
  ...
  const invData: GSTInvoiceData = {
    ...
    invoice_date: new Date(invoice.invoice_date || invoice.generated_at || invoice.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' }),
    ...
    taxable_value: Number(invoice.taxable_value) || 0,
    cgst_amount: invoice.is_inter_state ? 0 : (Number(invoice.cgst_amount) || 0),
    sgst_amount: invoice.is_inter_state ? 0 : (Number(invoice.sgst_amount) || 0),
    igst_amount: invoice.is_inter_state ? (Number(invoice.igst_amount) || 0) : 0,
  ...
  return (
    <div className="bg-slate-100 min-h-screen py-8">
      <div className="max-w-[210mm] mx-auto mb-4 flex justify-end no-print">
        <button onClick={() => window.print()} className="px-6 py-2 bg-indigo-600 text-white font-bold rounded shadow hover:bg-indigo-700">
          Print / Save as PDF
        </button>
      </div>
      <div className="w-full flex justify-center">
        <div className="w-full max-w-[900px] shadow-2xl">
          <GSTInvoiceTemplate invoice={invData} template={template} forPrint />
        </div>
      </div>
    </div>
  );
