...
export async function transitionOrder(
  ...
  if (nextStatus === 'DISPATCHED') {
    ...
    // Fix: Ensure all child items of this order also get marked as DISPATCHED in Supabase 
    // so they show up correctly in the Invoice Generation module.
    await supabaseServer
      .from('orders')
      .update({ status: 'DISPATCHED' })
      .like('id', `${parentId}-item%`);
...
async function executeOrderPlacementTx(
  ...
  // Generate IDs
  const baseId = payload.idOverride || generateOrderId();
  ...
  ledgerEntries.push({
    ...
    availableCredit: customerData.type === 'CREDIT' ? Math.max(0, creditLimit - balanceAfter) : 0,
    remarks: `Sale for Order ${baseId}`,
    createdBy: user.id
  });

  const shouldCreateReceipt = (user.role === 'ACDEMA' && customerData.type !== 'CREDIT' && payload.paymentMethod !== 'UPI' && payload.paymentMode !== 'UPI');
  ...
  if (shouldCreateReceipt) {
    ...
    ledgerEntries.push({
      ...
      availableCredit: 0,
      remarks: `Auto-verified proxy payment for Order ${baseId}`,
      createdBy: user.id,
      isVerified: true,
      verifiedBy: 'SYSTEM',
      verifiedAt: now,
        ...
        if (currentIdx >= 0 && currentIdx < snapshot.steps.length) {
          ...
          if (currentStep.role === role) {
            ...
            if (status === 'COMPLETED') {
              ...
              snapshot.steps[currentIdx].completedBy = user.id;
...
export async function advanceOrderWorkflow(orderId: string, notes?: string, metadata?: any) {
  ...
  if (nextStatus === freshData.status) {
    return; // No need to execute a status transition if it's already in the correct state
  }
...
export async function resumeWorkflowStep(orderId: string, notes?: string) {
  ...
  return await transitionOrder(orderId, 'IN_PROGRESS', notes || 'Resumed work step', user);
}
...
export async function dispatchOrder(orderId: string, dispatchInfo: any) {
  ...
  let result;
  if (currentStatus === 'DISPATCHED' || currentStatus === 'IN_TRANSIT' || currentStatus === 'DELIVERED') {
    await adminDb.collection('orders').doc(orderId).update(meta);
    result = { success: true };
  } else {
    result = await transitionOrder(orderId, 'DISPATCHED', 'Order Dispatched', user, meta);
  }
...
export async function completeTiffPrint(
  ...
  await advanceWorkflowSnapshotStep(orderId, 'PRINTER', 'COMPLETED', user, notes || 'Finished printing TIFF');
  
  const freshSnap = await adminDb.collection('orders').doc(orderId).get();
  const freshData = freshSnap.data() as any;
  const nextRole = freshData.currentWorkflowRole;
  
  let nextStatus: OrderStatus = freshData.status || 'IN_PROGRESS';
  if (nextRole === 'PRINTER' || nextRole === 'PASTING' || nextRole === 'FINISHING') {
    nextStatus = 'IN_PROGRESS';
  } else if (nextRole === 'DISPATCH') {
    nextStatus = 'COMPLETED';
  } else if (nextRole === 'DELIVERY') {
  ...
  if (nextStatus !== freshData.status) {
    await transitionOrder(orderId, nextStatus, 'Finished printing TIFF', user);
  }
