...
export async function generateInvoiceFromChildOrders(
  ...
  try {
    ...
    const rpcResult = (Array.isArray(rpcRows) ? rpcRows[0] : rpcRows) as any;
    ...
    if (coErr || !childOrders) throw new Error(coErr?.message || 'Child orders not found.');

    const orders: any[] = childOrders;
