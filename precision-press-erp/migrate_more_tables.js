...
async function run() {
  ...
  let additionalSql = '';
  try {
      additionalSql = fs.readFileSync('supabase/migrations/20260719051835_add_credit_notes_and_inventory.sql', 'utf-8');
  } catch(e) {}
    ...
    try {
        ...
        if (allRows.length > 0) {
            ...
            for(let i=0; i<allRows.length; i+=500) {
                ...
                // use upsert to avoid duplicate key errors if already migrated
                const { error: insertError } = await newDb.from(table).upsert(chunk);
                if (insertError) {
