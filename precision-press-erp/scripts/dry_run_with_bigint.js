...
async function run() {
  ...
  console.log('Testing dry-run with total_value as BIGINT...');
  await client.query('BEGIN');

  // Alter total_value to bigint inside transaction
  await client.query('ALTER TABLE public.inventory_item ALTER COLUMN total_value TYPE bigint;');

  // Fetch categories & GL accounts
  ...
  while ((m = itemRegex.exec(xml)) !== null) {
    ...
    `;
    const values = [
      DEFAULT_ORG_ID, code, sku, name, `Stock Item: ${name}${parent ? ` (${parent})` : ''}`,
      parent || 'General', categoryId, name, guid, alterId,
      parent || null, uom, altUom, billingMode,
      uom, purchasePricePaise, salePricePaise, purchasePricePaise, purchasePricePaise,
      quantityOnHand, openQty, openRate, openVal, totalValuePaise,
      inventoryAccountId, revenueAccountId, costAccountId,
      'average', 'none', isMandatory,
      defaultWidth, defaultLength, defaultWidthUnit, defaultLengthUnit, defaultSizeName,
      JSON.stringify({
        unit: uom,
  ...
  console.log(`?? ALL ${inserted} / 335 ITEMS INSERTED 100% CLEANLY WITH ZERO ERRORS!`);

  const countRes = await client.query('SELECT count(*) FROM public.inventory_item');
  console.log(`?? In-Transaction Item Count: ${countRes.rows[0].count}`);

  console.log('?? Executing ROLLBACK (Zero changes persisted)...');
  await client.query('ROLLBACK');
  console.log('? ROLLBACK complete.');

  const finalRes = await client.query('SELECT count(*) FROM public.inventory_item');
  console.log(`?? Post-Rollback Item Count: ${finalRes.rows[0].count} (Pristine state preserved)`);
