...
export async function generateInvoiceForParentOrder(baseOrderId: string) {
  try {
    ...
    if (!invoice) {
        ...
        const { data: pOrder } = await supabaseServer.from('orders').select('customer_id').eq('id', baseOrderId).single();
        ...
        const { data: newInvoice, error: insErr } = await supabaseServer.from('invoices').insert({
            ...
            customer_id: pOrder.customer_id,
    ...
    try {
        ...
        const childOrderIds = parentOrder.child_order_ids?.length > 0 ? parentOrder.child_order_ids : [baseOrderId];
        ...
        const deliveryChoice = parentOrder.delivery_choice || 'PICKUP';
        const deliveryPricing = parentOrder.delivery?.pricingSnapshot || {};
        let transport = new Decimal(0);
        if (parentOrder.delivery_choice === 'DOOR_DELIVERY') transport = new Decimal(deliveryPricing.door || 0);
        else if (parentOrder.delivery_choice === 'COURIER') transport = new Decimal(deliveryPricing.courier || 0);
        else if (parentOrder.delivery_choice === 'TRANSPORT') transport = new Decimal(deliveryPricing.transport || 0);
        ...
        const hashPayload = JSON.stringify({
          ...
          financial_year: invoice.financial_year,
            invoice_date: now.toISOString().split('T')[0],
          customer_snapshot: parentOrder.customer_snapshot || {},
        ...
        const updatePayload = {
          ...
          tax_details: taxDetails,
          status: 'GENERATED',
          generated_by: parentOrder.created_by || 'SYSTEM',
          order_type: parentOrder.order_type || 'CASH',
        ...
        await supabaseServer.from('invoice_events').insert({
           ...
           performed_by: parentOrder.created_by || 'SYSTEM',
