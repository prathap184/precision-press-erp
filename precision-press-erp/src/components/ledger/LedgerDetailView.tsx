...
interface Transaction {
  ...
  timestamp: any;
  isVerified?: boolean;
  sale_entry_number?: string;
  receipt_entry_number?: string;
}
  ...
  useEffect(() => {
    ...
    const unsubscribeTx = onSnapshot(
      ...
      (snapshot) => {
        ...
        const refs = [...new Set(data.map(t => t.refId || t.sale_entry_number).filter(Boolean))] as string[];
        if (refs.length > 0) {
          supabase
            .from('invoices')
            .select('id, invoice_number')
            .in('invoice_number', refs)
            .then(({ data: invData }) => {
              if (invData) {
                const idMap: Record<string, string> = {};
                invData.forEach(inv => {
                  if (inv.invoice_number && inv.id) {
                    idMap[inv.invoice_number] = inv.id;
                  }
                });
                setInvoiceIds(idMap);
              }
                  ...
                  {filtered.map((tx) => (
                    <tr key={tx.id} className="group hover:bg-slate-50 transition-colors h-9">
                      <td className="border border-slate-300 px-2.5 py-2 align-top tabular-nums">
                        ...
                        <p className="text-[8px] font-bold text-slate-400 italic mt-0.5">
                          {tx.type === 'RECEIPT' ? (
                            <Link href={`/receipt-entry/${tx.id}`} className="hover:underline text-blue-500">
                              {tx.receipt_entry_number || `TX_${tx.id.slice(-6).toUpperCase()}`}
                            </Link>
                          ) : tx.type === 'SALE' && invoiceIds[tx.sale_entry_number || tx.refId] ? (
                            <Link href={`/admin/invoices/${invoiceIds[tx.sale_entry_number || tx.refId]}/print`} className="hover:underline text-blue-500">
                              {tx.sale_entry_number || `TX_${tx.id.slice(-6).toUpperCase()}`}
                            </Link>
                          ) : (
                            tx.sale_entry_number || tx.receipt_entry_number || `TX_${tx.id.slice(-6).toUpperCase()}`
                          )}
                        </p>
                      </td>
                      <td className="border border-slate-300 px-2.5 py-2 align-top tabular-nums">
                      ...
                      <td className="border border-slate-300 px-2.5 py-2 align-top tabular-nums">
                        {tx.refId?.startsWith('ORD-') ? (
                          ...
                          <Link
                            href={`/admin/invoices/${invoiceIds[tx.refId]}/print`}
                            className="text-[10px] font-bold text-blue-600 hover:underline flex items-center gap-1"
                          >
                            {tx.refId}
                          </Link>
