                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b-2 border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400">
                        ...
                        <th className="py-2.5 px-2">File Path *</th>
                          ...
                          <tr key={row.id} className="group transition-colors hover:bg-slate-50/50 align-top">
                            ...
                            <td className="py-2 px-2 tabular-nums align-top">
                              {(() => {
                                ...
                                return (
                                  <div id={`error-row-${row.id}-product`} className="relative w-full min-w-[220px]">
                                    <div className={`flex h-10 w-full items-center rounded-lg px-3 border transition-all duration-150 ${validationErrors[`row-${row.id}-product`] ? 'border-2 border-red-500 ring-4 ring-red-500/30 bg-red-50/50 shadow-md' : isOpen ? 'border-2 border-blue-600 bg-white ring-4 ring-blue-500/20 shadow-sm' : activeDescRowId ? 'border-2 border-slate-200 bg-slate-50' : 'border-2 border-slate-200 bg-slate-50 focus-within:border-blue-600 focus-within:ring-4 focus-within:ring-blue-500/20 focus-within:bg-white'}`}>
                                         <input
                                           ...
                                           onKeyDown={(e) => {
                                             if (e.key === "ArrowDown") {
                                               ...
                                               if (selectedProduct) {
                                                 const prodMode = (selectedProduct as any)?.tally_billing_mode || (selectedProduct as any)?.tallyBillingMode || 'B';
                                               ...
                                               if (displayedItems.length > 0) {
                                                 const selectedProduct = displayedItems[0];
                                                 const prodMode = (selectedProduct as any)?.tally_billing_mode || (selectedProduct as any)?.tallyBillingMode || 'B';
                                                 updateRow(row.id, { productId: selectedProduct.id, billingMode: prodMode });
                                                 setValidationErrors((prev: any) => { const n = { ...prev }; delete n[`row-${row.id}-product`]; return n; });
                                                 setOpenRowId(null);
                                    ...
                                    {/* Additional Description for Stock Item (Tally Prime) */}
                            ...
                            <td className="py-2 px-2 text-center tabular-nums align-top">
                              {hasMultipleSizes && isModeA ? (
                                ...
                                <div className="h-10 inline-flex items-center justify-center gap-1">
                                  ...
                                  />
                                  <span className="text-[10px] text-slate-500 font-bold">{isSqft ? 'sqft' : displayUnit}</span>
                                </div>
                              )}
                            </td>
                            <td className="py-2 px-2 tabular-nums align-top">
            ...
            >
              {customerDropdownOpen ? (
                <>
                  ...
                  <div className="bg-[#e2f0fd] border-b border-[#bad5f5] py-1 px-4 flex items-center justify-between shrink-0">
                    <button
                      type="button"
                      onMouseDown={(e) => {
                        e.preventDefault();
