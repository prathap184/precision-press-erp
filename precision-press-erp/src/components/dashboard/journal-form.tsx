...
export function JournalForm({ 
  ...
  const [subType, setSubType] = useState(initial?.subType || "General Journal");
      ...
      <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
        <div className="flex flex-col space-y-1.5 p-6 pb-4 border-b bg-muted/20">
          <div className="flex justify-between items-center">
            <h3 className="font-semibold leading-none tracking-tight text-lg">Journal Voucher</h3>
      ...
      <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); handleSubmit("posted"); }}>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-2">
            <Label htmlFor="subType">Journal Type</Label>
            <Select value={subType} onValueChange={setSubType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Customer Transfer">Customer Transfer</SelectItem>
                <SelectItem value="Supplier Transfer">Supplier Transfer</SelectItem>
                <SelectItem value="Tax Adjustment">Tax Adjustment</SelectItem>
        ...
        <div className="space-y-3">
          <div className="overflow-x-auto rounded-lg border shadow-sm">
            <div className="grid min-w-[1200px] grid-cols-[200px_150px_150px_150px_1fr_120px_120px_40px] gap-2 border-b bg-muted/50 px-3 py-2 text-xs font-semibold text-muted-foreground">
              <span>Account</span>
              <span>Contact (Opt)</span>
              <span>Cost Center (Opt)</span>
              <span>Project (Opt)</span>
            ...
            {lines.map((line, i) => (
              <div
                key={i}
                className="grid min-w-[1200px] grid-cols-[200px_150px_150px_150px_1fr_120px_120px_40px] gap-2 border-b px-3 py-2 last:border-b-0 hover:bg-muted/10 transition-colors items-center"
              >
                <Select
                ...
                <Select
                  value={line.contactId || "none"}
                  onValueChange={(v) => updateLine(i, "contactId", v === "none" ? "" : v)}
                >
                  <SelectTrigger className="h-8 text-sm bg-background">
                    <SelectValue placeholder="No contact" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none" className="text-muted-foreground">No contact</SelectItem>
                    {contacts.map((c) => (
                ...
                <Select
                  ...
                  onValueChange={(v) => updateLine(i, "costCenterId", v === "none" ? "" : v)}
                >
                  <SelectTrigger className="h-8 text-sm bg-background">
                    <SelectValue placeholder="No CC" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none" className="text-muted-foreground">No CC</SelectItem>
                    {costCenters.map((c) => (
                      <SelectItem key={c.id} value={c.id}>
                        {c.name}
                      </SelectItem>
                ...
                <Select
                  ...
                  onValueChange={(v) => updateLine(i, "projectId", v === "none" ? "" : v)}
                >
                  <SelectTrigger className="h-8 text-sm bg-background">
          ...
          <div className="flex justify-end">
             {isBalanced ? (
               <div className="flex items-center text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-md border border-emerald-200">
                 <CheckCircle2 className="size-4 mr-2" />
                 <span className="text-sm font-medium">Voucher Balanced</span>
               </div>
             ) : (
               <div className="flex items-center text-destructive bg-destructive/10 px-3 py-1.5 rounded-md border border-destructive/20">
