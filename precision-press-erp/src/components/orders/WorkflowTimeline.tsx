                ...
                {entry.metadata && Object.keys(entry.metadata).length > 0 && (
                  <div className="mt-3 space-y-3">
                    ...
                    {Object.entries(entry.metadata).map(([key, value]) => {
                      if (typeof value === 'string' && value && (key.toLowerCase().includes('proof') || value.match(/\.(jpeg|jpg|gif|png|webp)/i))) {
                        return (
                          <div key={key} className="inline-block mr-3 mb-2 align-top">
                            <p className="text-[9px] font-black uppercase text-slate-500 mb-1.5 tracking-wider">{key.replace('workflow.', '')}</p>
                            <a href={value} target="_blank" rel="noreferrer" className="block transition-transform hover:scale-105">
                              <img src={value} alt={key} className="max-h-32 rounded-lg object-contain border border-slate-200 shadow-sm bg-white" />
                            </a>
                          </div>
                        );
                      }
                    ...
                    {/* Render Raw JSON for other metadata */}
                    <div className="bg-slate-50 rounded-lg p-2.5 text-[10px] text-slate-600 border border-slate-100 overflow-x-auto">
