        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {loading ? 'Processing...' : isComplete ? 'Confirm Send to Next Stage' : isRedesign ? 'Send Redesign Request' : 'Send for Verification'}
            </button>
      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={item.id} className="rounded-[2rem] border border-white/40 bg-white/30 backdrop-blur-2xl p-6 space-y-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
            ...
            {(item.designType === 'COMPANY_DESIGN' || item.designUrl === 'DESIGN_BY_US' || item.fileUrl === 'DESIGN_BY_US' || item.itemWorkspace?.customerUploadUrl === 'DESIGN_BY_US') && !hasCorrectedArtwork && (
...
export function DesignerOrderWorkspace({ orderId, itemId }: DesignerOrderWorkspaceProps) {
  ...
  const guard = useStageWorkspaceGuard('DESIGNER', order, loading);
  ...
  useEffect(() => {
    ...
    const q = doc(db, 'orders', candIds[0]);
    ...
    itemsUnsub = onSnapshot(collection(db, 'orders', candIds[0], 'items'), (itemsSnap) => {
  ...
  const handleReturnRedirect = () => {
    ...
    try {
      ...
      setTimeout(() => router.push(returnTo || guard.fallbackUrl), 700);
    }
  };
  ...
  const hasCompanyDesign = displayItems.some(i => i.designType === 'COMPANY_DESIGN' || i.designUrl === 'DESIGN_BY_US' || i.fileUrl === 'DESIGN_BY_US' || i.itemWorkspace?.customerUploadUrl === 'DESIGN_BY_US');

  // Check if any items are missing an upload (Mandatory unless customer provided a valid file)
  const hasUnuploadedDesigns = displayItems.some(i => {
    const designerUpload = i.itemWorkspace?.designerUploadUrl;
    const hasValidDesignUrl = i.designUrl && i.designUrl !== 'DESIGN_BY_US';
    const hasCustomerFile = i.fileUrl && i.fileUrl !== 'DESIGN_BY_US';
    
    return !(designerUpload || hasValidDesignUrl || hasCustomerFile);
  });
  ...
  if (hasUnuploadedDesigns) {
    ...
    blockReason = 'You must upload a design for all items first';
  } else {
