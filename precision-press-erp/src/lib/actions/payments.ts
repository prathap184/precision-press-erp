...
export async function submitPayment(data: {
  ...
  try {
    ...
    if (isGroupedOrder && data.baseOrderId) {
      revalidatePath(`/customer/payment/group/${data.baseOrderId}`);
      data.orderIds!.forEach(id => revalidatePath(`/customer/payment/${id}`));
    } else {
      revalidatePath(`/customer/payment/${data.orderId}`);
    }
...
export async function approvePayment(
  ...
  try {
    ...
    if (paymentId.startsWith('V-CREDIT-')) {
      ...
      orderIdsToProcess.forEach((id) => revalidatePath(`/customer/payment/${id}`));
      return { success: true };
    }
      ...
      // --- Handle Standard Payments ---
        ...
        revalidatePath('/accountant/payments');
        revalidatePath('/accountant/ledger');
        orderIdsToProcess.forEach((id: string) => revalidatePath(`/customer/payment/${id}`));
        return { success: true };
  } catch (error: any) {
...
export async function approveSupplierPaymentRequest(id: string) {
  try {
    const { uid, roles } = await getAuthUser();
    ...
    const payRef = adminDb.collection('payments').doc(id);
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      // 2. Adjust supplier credit (negative for them = credit added or advance given)
      const supplierId = data?.userId;
      if (supplierId && data?.amount) {
        const profRef = adminDb.collection('profiles').doc(supplierId);
        const profSnap = await tx.get(profRef);
        if (profSnap.exists) {
          const currentCredit = profSnap.data()?.usedCredit || 0;
          tx.update(profRef, { usedCredit: currentCredit - data.amount });
        }
      }
    });
...
export async function rejectSupplierPaymentRequest(id: string, reason: string) {
  try {
    const { uid, roles } = await getAuthUser();
    if (!roles.includes('ADMIN') && !roles.includes('SUPER_ADMIN')) {
      return { success: false, error: 'Permission denied' };
