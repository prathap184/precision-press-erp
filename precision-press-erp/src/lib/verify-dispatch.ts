...
import { transitionOrder } from '@/lib/workflow';
// Invoice generation is now manual — removed generateInvoiceForParentOrder import

import { randomUUID } from 'crypto';

// Minimal mock user
const MOCK_USER = { id: 'SYSTEM_ADMIN', name: 'System Admin', role: 'ADMIN' as any };

async function resetOrder(orderId: string, status: string) {
    await adminDb.collection('orders').doc(orderId).update({ status });
}

async function logTest(name: string, fn: () => Promise<boolean>) {
    try {
...
async function runTests() {
    ...
    await logTest('Scenario 1: PLACED → DISPATCHED generates invoice', async () => {
        // Transition to DISPATCHED
        await transitionOrder(testOrderId, 'DISPATCHED', 'Test Dispatch 1', MOCK_USER);
        
        // Wait for background logic (since workflow.ts does it inline async)
        await new Promise(r => setTimeout(r, 2000));
        
        const { data: invs } = await supabaseServer.from('invoices').select('*').eq('parent_order_id', testOrderId);
        if (!invs || invs.length !== 1) return false;
    ...
    await logTest('Scenario 2: DISPATCHED → DISPATCHED (no new invoice)', async () => {
        // Mock UI updating dispatch fields while still in DISPATCHED status
        await transitionOrder(testOrderId, 'DISPATCHED', 'Test Dispatch 2', MOCK_USER);
        await new Promise(r => setTimeout(r, 2000));
    ...
    await logTest('Scenario 3: DISPATCHED → IN_PROGRESS → DISPATCHED (no new invoice)', async () => {
        await resetOrder(testOrderId, 'IN_PROGRESS');
        await transitionOrder(testOrderId, 'DISPATCHED', 'Test Dispatch 3', MOCK_USER);
        await new Promise(r => setTimeout(r, 2000));
    ...
    await logTest('Scenario 4: DISPATCH_ROLLED_BACK → DISPATCHED (reuse existing row & number)', async () => {
        // Rollback
        await supabaseServer.from('invoices').update({ status: 'DISPATCH_ROLLED_BACK' }).eq('parent_order_id', testOrderId);
        await resetOrder(testOrderId, 'IN_PROGRESS');
        
        // Redispatch
        await transitionOrder(testOrderId, 'DISPATCHED', 'Test Dispatch 4', MOCK_USER);
        await new Promise(r => setTimeout(r, 2000));
    ...
    await supabaseServer.from('invoices').delete().eq('parent_order_id', testOrderId);
}
