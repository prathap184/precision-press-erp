...
import { approveSupplierPaymentRequest, rejectSupplierPaymentRequest } from '@/lib/actions/payments';
...
export default function PaymentRequestsClient({ initialRequests }: PaymentRequestsClientProps) {
  ...
  const handleApprove = async (id: string) => {
    if (!confirm('Are you sure you want to approve this request and add credit to the supplier?')) return;
    
    setLoadingAction(id);
    const result = await approveSupplierPaymentRequest(id);
  ...
  const handleReject = async (id: string) => {
    ...
    setLoadingAction(id);
    const result = await rejectSupplierPaymentRequest(id, reason);
    setLoadingAction(null);
            ...
            <table className="w-full text-sm text-left border-collapse">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold text-gray-600">Supplier</th>
