...
export default function InvoiceGenerationOrderList({ params }: { params: { customerId: string } }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // For each parent order, fetch child order count and invoiced count
        const enriched: ParentOrderRow[] = await Promise.all(
          pOrders.map(async (po: any) => {
            const { data: children, error: childErr } = await supabase
              .from('orders')
              .select('id, invoice_generated')
              .like('id', `${po.id}-item%`);

            const childList = !childErr && children ? children : [];
            const totalChildOrders   = childList.length;
            const invoicedChildOrders = childList.filter((c: any) => c.invoice_generated).length;
  ...
  const getInvoiceProgressColor = (invoiced: number, total: number) => {
    if (total === 0) return 'text-slate-400';
    if (invoiced === 0) return 'text-amber-600';
    if (invoiced === total) return 'text-green-600';
    return 'text-blue-600';
  };

  const getInvoiceProgressBg = (invoiced: number, total: number) => {
    if (total === 0) return 'bg-slate-100';
    if (invoiced === 0) return 'bg-amber-50';
    if (invoiced === total) return 'bg-green-50';
    return 'bg-blue-50';
  };
    ...
    <RoleGuard allowedRoles={['ADMIN', 'SUPER_ADMIN', 'ACCOUNTANT']}>
      <div className="max-w-5xl mx-auto space-y-6 pb-12">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => router.push('/admin/invoice-generation')}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {orders.map(order => {
              const progressColor = getInvoiceProgressColor(order.invoicedChildOrders, order.totalChildOrders);
              const progressBg    = getInvoiceProgressBg(order.invoicedChildOrders, order.totalChildOrders);
              const fullyInvoiced = order.totalChildOrders > 0 && order.invoicedChildOrders === order.totalChildOrders;
              ...
              return (
                <Link
                  key={order.id}
                  href={`/admin/invoice-generation/${customerId}/${order.id}`}
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${fullyInvoiced ? 'bg-green-100' : 'bg-indigo-50'}`}>
                        {fullyInvoiced
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className={`px-3 py-2 rounded-xl text-center ${progressBg}`}>
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-0.5">Items Invoiced</p>
                        <p className={`text-sm font-black ${progressColor}`}>
                          {order.totalChildOrders > 0
                            ? `${order.invoicedChildOrders} / ${order.totalChildOrders}`
                            : '—'
                          }
                        </p>
                      </div>
