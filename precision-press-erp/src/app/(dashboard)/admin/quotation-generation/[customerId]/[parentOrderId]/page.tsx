...
import GSTQuotationTemplate, { TaxTemplate, GSTQuotationData } from '@/components/documents/GSTQuotationTemplate';
...
export default function QuotationGenerationSelectPage({
  params,
}: {
  ...
  const [genSuccess, setGenSuccess] = useState<{ quotationId: string; quotationNumber: string } | null>(null);
    ...
    try {
      ...
      if (tpl) {
        setCompanyTemplate({
          ...
          quotation_prefix: tpl.quotation_prefix,
  ...
  const selectAllPending = () => {
    const pendingIds = childOrders
      .filter(c => !c.quotation_generated && (c.quotation_status === 'PENDING' || !c.quotation_status))
      .map(c => c.id);
  ...
  const pendingCount = childOrders.filter(c => !c.quotation_generated).length;
  const quotationdCount = childOrders.filter(c => c.quotation_generated).length;
  ...
  const handleGenerate = async () => {
    ...
    if (hasMixed) {
      setGenError('Cannot mix interstate (IGST) and intrastate (CGST+SGST) items in a single quotation.');
      return;
    }
    setGenerating(true);
    setGenError('');
    try {
      ...
      if (!data.success) throw new Error(data.error || 'Quotation generation failed');
      setGenSuccess({ quotationId: data.quotationId, quotationNumber: data.quotationNumber });
      await loadData();
      setSelected(new Set());
    } catch (err: any) {
    ...
    <RoleGuard allowedRoles={['ADMIN', 'SUPER_ADMIN', 'ACCOUNTANT']}>
      <div className="max-w-6xl mx-auto space-y-6 pb-12">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => router.push(`/admin/quotation-generation/${customerId}`)}
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-slate-400 flex-wrap">
          ...
          <Link href={`/admin/quotation-generation/${customerId}`} className="text-violet-600 hover:underline">{customerName}</Link>
          <ChevronRight size={12} />
        ...
        {genSuccess && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-black text-green-800">Quotation Generated Successfully!</p>
                <p className="text-xs text-green-600 font-mono font-bold mt-0.5">{genSuccess.quotationNumber}</p>
                <p className="text-xs text-green-600 font-medium mt-1">
                  Select more items above to generate another quotation for this order.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <Link
                href={`/admin/quotation-register/${genSuccess.quotationId}`}
              ...
              <div className="flex items-center justify-between bg-white rounded-2xl border border-slate-200 shadow-sm px-5 py-3">
                <div className="flex items-center gap-4">
                  <div className="text-xs font-bold text-slate-500">
                    ...
                    <span className="text-green-600">{quotationdCount}</span> Quotationd
                  </div>
                  {selected.size > 0 && (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50">
                        ...
                        <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Quotation</th>
                        <th className="w-10 px-4 py-3" />
                      </tr>
                    </thead>
                    <tbody>
                      {childOrders.map((child, idx) => {
                        const isPending = !child.quotation_generated;
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-4 text-center">
                              <StatusBadge status={child.quotation_generated ? (child.quotation_status || 'GENERATED') : 'PENDING'} />
                            </td>

                            {/* Quotation Number */}
                            <td className="px-4 py-4 text-center">
                              {child.quotation_number ? (
                                <span className="text-[10px] font-black text-slate-700 font-mono">{child.quotation_number}</span>
                              ) : (
                            ...
                            <td className="px-4 py-4 text-center">
                              {child.quotation_id ? (
                                <Link
                                  href={`/admin/quotation-register/${child.quotation_id}/print`}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Quotation Summary</h3>
                </div>
                ...
                {hasMixed && (
                  <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                    ...
                    <p className="text-xs text-amber-700 font-medium">
                      Cannot mix interstate (IGST) and intrastate (CGST+SGST) items in one quotation.
                    </p>
                  </div>
                )}
                ...
                {hasUndispatched && (
                  <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                    <AlertCircle size={14} className="text-amber-600 flex-shrink-0 mt-0.5" />
                    <p className="text-xs text-amber-700 font-medium">
                      Cannot generate quotation. Some selected items have not been dispatched yet (dispatch details missing).
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-full overflow-y-auto shadow-2xl relative flex flex-col">
            <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10 rounded-t-3xl">
              <h2 className="text-lg font-black text-slate-900 uppercase">Quotation Preview</h2>
              ...
              <div className="w-full shadow-lg rounded-sm overflow-hidden" style={{ maxWidth: '900px' }}>
                {companyTemplate ? (() => {
                  ...
                  <div className="p-12 text-center text-slate-500">
                    ...
                    <p>No active quotation template found. Please configure GST & Quotation settings.</p>
                  </div>
                )}
              </div>
