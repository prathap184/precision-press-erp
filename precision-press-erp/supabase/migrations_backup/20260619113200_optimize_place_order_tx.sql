-- SQL Migration: Optimize place_order_tx with set-based operations
-- Replaces FOREACH loops with unnest() for improved performance on bulk inserts.
-- Consolidates NOW() calls into a single timestamp to avoid multiple clock reads.
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    VALUES (p_idempotency_key, p_parent_order->>'id', v_now);
  END IF;
