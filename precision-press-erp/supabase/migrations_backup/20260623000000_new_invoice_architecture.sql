-- ============================================================
-- New Invoice Architecture Migration
-- Replaces automatic-dispatch invoice generation with
-- manual Invoice Generation module.
-- ============================================================

-- 1. Add immutable financial snapshot columns to the orders table
--    These are set once at child order creation and never modified.
--    NULL = order created before this migration (handled gracefully by UI).
...
ALTER TABLE public.orders
  ...
  ADD COLUMN IF NOT EXISTS gst_type                TEXT,          -- 'CGST_SGST' | 'IGST'
  ADD COLUMN IF NOT EXISTS allocated_logistics_percentage NUMERIC(8,4),
  ADD COLUMN IF NOT EXISTS allocated_logistics_amount     NUMERIC(12,2),
  ADD COLUMN IF NOT EXISTS item_amount             NUMERIC(12,2),
  ADD COLUMN IF NOT EXISTS taxable_value_snapshot  NUMERIC(12,2),
  ADD COLUMN IF NOT EXISTS grand_total_snapshot    NUMERIC(12,2),
  -- Invoice link fields (set when invoice is generated for this child order)
  ADD COLUMN IF NOT EXISTS invoice_id              TEXT,
  ADD COLUMN IF NOT EXISTS invoice_number          TEXT,
  ADD COLUMN IF NOT EXISTS invoice_generated       BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS invoice_generated_at    TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS invoice_status          TEXT DEFAULT 'PENDING';  -- PENDING | GENERATED | PRINTED | HANDED_OVER | CANCELLED

-- Index for fast lookups in Invoice Generation module
CREATE INDEX IF NOT EXISTS idx_orders_invoice_generated ON public.orders (invoice_generated);
CREATE INDEX IF NOT EXISTS idx_orders_invoice_id ON public.orders (invoice_id);
CREATE INDEX IF NOT EXISTS idx_orders_customer_id_status ON public.orders ("customerId", status);

-- 2. Revamp atomic_dispatch_order RPC
--    Remove invoice number reservation entirely.
--    Dispatch now only marks the order DISPATCHED — nothing more.
...
BEGIN
  ...
  -- 3. Mark as dispatched — no invoice number reserved here
...
-- 3. New atomic invoice generation RPC
--    Called by the Invoice Generation module when staff clicks "Generate Invoice".
--    Assigns the next sequential invoice number and atomically links it to
--    the selected child orders.
...
-- 4. Add transport_amount column to invoices if missing (for summary storage)
ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS transport_amount NUMERIC(12,2) DEFAULT 0,
  ADD COLUMN IF NOT EXISTS child_order_ids  TEXT[]        DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS item_count       INTEGER       DEFAULT 0;

-- Index for querying invoices by parent order quickly
CREATE INDEX IF NOT EXISTS idx_invoices_parent_order_new ON public.invoices (parent_order_id);
