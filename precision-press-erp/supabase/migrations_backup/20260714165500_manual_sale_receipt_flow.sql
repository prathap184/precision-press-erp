-- Migration to support manual Sale and Receipt entries

-- 1. Add current_recharge to profiles
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS current_recharge numeric DEFAULT 0;

-- 2. Add entry numbers to orders
ALTER TABLE orders ADD COLUMN IF NOT EXISTS sale_entry_number text;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS receipt_entry_number text;

-- 3. Update place_order_tx to conditionally update usedCredit ONLY if a SALE transaction is included
...
BEGIN
  ...
  -- Check what ledger entries were passed
  IF p_ledger_entries IS NOT NULL THEN
    FOR ledger_item IN SELECT * FROM unnest(p_ledger_entries) LOOP
      IF ledger_item->>'type' = 'SALE' THEN
        v_has_sale := true;
      END IF;
      IF ledger_item->>'type' = 'RECEIPT' THEN
        v_has_receipt := true;
        v_receipt_amount := v_receipt_amount + COALESCE((ledger_item->>'credit')::numeric, 0);
      END IF;
    END LOOP;
  END IF;
  ...
  -- 3 & 4. Validate and Update Credit Balance ONLY if SALE is present
  IF v_has_sale THEN
    ...
    -- If there's also an immediate receipt, subtract it
    IF v_has_receipt THEN
      v_new_used_credit := v_new_used_credit - v_receipt_amount;
    END IF;
