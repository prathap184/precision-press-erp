-- SQL Migration: place_order_tx PL/pgSQL Function
-- Executes the entire parent/child order placement, ledger inserts, credit checks, and job queueing in a single ACID transaction.
...
BEGIN
  ...
  UPDATE profiles
  SET "usedCredit" = v_new_used_credit, "updatedAt" = NOW()
  WHERE id = p_customer_id;

  -- 4. Insert Parent Order (if provided)
  -- If N=1, the parent order is the single order.
  -- If N>1, the parent order is the grouping container.
  IF p_parent_order IS NOT NULL THEN
    INSERT INTO orders (
      ...
      COALESCE(p_parent_order->>'createdAt', NOW()::text),
      COALESCE(p_parent_order->>'updatedAt', NOW()::text),
  ...
  IF p_child_orders IS NOT NULL THEN
    FOREACH child_val IN ARRAY p_child_orders LOOP
      INSERT INTO orders (
        ...
        COALESCE(child_val->>'createdAt', NOW()::text),
        COALESCE(child_val->>'updatedAt', NOW()::text),
  ...
  IF p_ledger_entries IS NOT NULL THEN
    FOREACH ledger_val IN ARRAY p_ledger_entries LOOP
      INSERT INTO transactions (
        ...
        CASE WHEN ledger_val->>'verifiedAt' IS NOT NULL THEN (ledger_val->>'verifiedAt')::timestamptz ELSE NULL END,
