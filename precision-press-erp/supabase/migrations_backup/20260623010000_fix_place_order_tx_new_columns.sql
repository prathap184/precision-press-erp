-- SQL Migration: Update place_order_tx to include new financial snapshot columns
-- Adds cgst_percentage, cgst_amount, sgst_percentage, sgst_amount, igst_percentage, igst_amount, 
...
BEGIN
  ...
  IF p_ledger_entries IS NOT NULL THEN
    ...
    SELECT
      ...
      COALESCE(ledger_val->>'timestamp', v_now::text)::timestamp with time zone,
      COALESCE((ledger_val->>'isVerified')::boolean, false),
      (ledger_val->>'verifiedAt')::timestamp with time zone,
      ledger_val->>'verifiedBy',
      ledger_val->>'paymentId'
    FROM unnest(p_ledger_entries) AS ledger_val;
  END IF;

  -- 9. Insert Jobs into job_queue table (Batch)
  IF p_jobs IS NOT NULL THEN
    INSERT INTO job_queue (
      id, job_type, order_id, priority, payload, status, created_at, updated_at
    )
    SELECT
      job_val->>'jobId',
      job_val->>'jobType',
      job_val->>'orderId',
      COALESCE((job_val->>'priority')::integer, 0),
