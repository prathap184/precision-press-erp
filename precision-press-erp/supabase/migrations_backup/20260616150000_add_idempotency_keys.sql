-- SQL Migration: Add idempotency_keys table and update place_order_tx
-- Establishes database-level idempotency checks inside the placement transaction.
...
-- Drop the previous version of the function to allow changing signature (adding p_idempotency_key parameter)
DROP FUNCTION IF EXISTS place_order_tx(
  p_customer_id text,
  p_order_type text,
...
-- Recreate place_order_tx with p_idempotency_key parameter
...
BEGIN
  ...
  -- 9. Insert Background Jobs into document_jobs table (Batch)
  ...
  -- 10. Record the Idempotency Key
  IF p_idempotency_key IS NOT NULL THEN
    INSERT INTO idempotency_keys (idempotency_key, order_id, created_at)
    VALUES (p_idempotency_key, p_parent_order->>'id', NOW());
  END IF;
